...
public class KlijentiFizickaLica extends Controller{
	...
	public static void filter(KlijentFizickoLice klijent, String mode){
		List<KlijentFizickoLice> klijenti = KlijentFizickoLice.find("ime like ? AND prezime like ? AND adresa like ? AND naseljenomesto_id = ?",  "%"+klijent.ime+"%", "%"+klijent.prezime+"%", "%"+klijent.adresa+"%", klijent.naseljenoMesto.id).fetch();
